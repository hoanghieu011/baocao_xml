...
@Component({
  selector: 'app-bao-cao-bo-phan',
  standalone: true,
  imports: [FormsModule, CommonModule, Select2Module, TranslateModule],
  templateUrl: './bao-cao-bo-phan.component.html',
  styleUrl: './bao-cao-bo-phan.component.css'
})
export class BaoCaoBoPhanComponent implements OnInit {
  ...
  exportExcel() {
    ...
    this.baoCaoService.xuatBaoCaoNghiPhepBoPhan(request).subscribe({
      next: (blob: Blob) => {
  ...
  searchBaoCao() {
    ...
    this.baoCaoService.getBaoCaoNghiPhepBoPhan(request)
      .subscribe({
        next: (response) => {
